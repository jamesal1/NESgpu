...
class Binarized(Perturbed, nn.Module):
    ...
    def set_noise(self, noise_scale=None):
        ...
        self.weight_noise = booleanOperations.sample_bits(torch.sigmoid(self.weight), self.directions, self.dtype.bits, self.seed)
    ...
    def set_grad(self, weights):
        ...
        self.weight.grad = booleanOperations.binary_weighted_sum(self.weight_noise, weights, self.in_degree)
...
class BinarizedLinear(Binarized):
    ...
    def __init__(self, in_features, out_features,  directions, threshold=True, dtype=torch.int64, device="cuda"):
        Binarized.__init__(in_features, out_features,  directions, threshold, dtype, device)
    ...
    def set_noise(self, noise_scale=None):
        ...
        if noise_scale is not None:
            self.set_noise_scale(noise_scale)
        self.weight_noise = boolop_cuda.sample_bits(torch.sigmoid(self.weight), self.directions, self.dtype.bits, self.seed)
        self.bias_noise = self.bias + (torch.rand(size=(self.directions, self.out_degree), dtype=torch.float16,
                                                  device=self.weight.device) - .5) * self.noise_scale
...
class BinarizedConv2d(Binarized):
    ...
    def __init__(self,in_channels, out_channels,  directions, kernel_size, stride=1,
        ...
        Binarized.__init__(in_channels, out_channels,  directions, threshold, dtype, device)
        ...
        self.weight = Parameter(torch.zeros(out_channels * kernel_size[0] * kernel_size[1], in_channels, dtype=torch.float16, device=device))
        if threshold:
    ...
    def set_noise(self, noise_scale=None):
        ...
        self.weight_noise = self.weight_noise.view(self.directions, self.out_channels, *self.kernel_size, -1)
        self.weight_noise = self.weight_noise.permute([0,2,3,4,1]).view(self.directions, -1, self.weight_noise.size(1)).contiguous()

    def forward(self, input):
        if self.perturbed_flag:
            packed_input = booleanOperations.pack(input, self.dtype) if input.dtype == torch.bool else input
            if self.threshold:
                return booleanOperations.conv_act(packed_input, self.weight_noise,self.threshold, self.padding, self.padding, self.stride, self.stride)
            return booleanOperations.conv(packed_input, self.weight_noise,self.threshold, self.padding, self.padding, self.stride, self.stride)
        else:
            raise NotImplementedError
...
class BinarizedMaxPool2d(nn.Module):
    ...
    def __init__(self, filter_size=(2,2),padding=(0,0),stride=(2,2),dtype=torch.int64):
        nn.Module.__init__(self)
    ...
    def forward(self, input):
        packed_input = booleanOperations.pack(input, self.dtype) if input.dtype == torch.bool else input
        return booleanOperations.max_pool2d(packed_input, *self.filter_size,*self.padding, *self.stride)